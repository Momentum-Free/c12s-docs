...
The 'Calibration Mode' toggle allows you to turn on AI Policies and Pattern Rules, but without actually executing their respective punishment actions.
Instead, it'll only log that it would've done so if the calibration mode was off, so make sure you have Logging enabled in the Org. Settings. This is often called doing a "dry run" in IT terms/lingo.
