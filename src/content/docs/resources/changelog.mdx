...
- New internal feature for 'sanitizing user questions' -- this increases the level of matches from the knowledge base, specially if the user asks something super long (or with multiple questions).
  - Unfortunately, however, this has made the AI too 'trigger happy', so we're adding yet another step (for paid plans) to rerank the KB outputs and mark the results as relevant or not relevant to the user's original question (if all results are irrelevant, the AI will not answer, to avoid spamming the chat when unmprompted).
