...
**People & containers:**  
[Organization](#organization) · [Organization Members](#organization-members) · [Company](#company-billing)

**Community & platforms:**  
[Community](#community) · [Community Platform](#community-platform) · [Community Rules](#community-rules)
...
**Control & safety:**  
[Overrides](#overrides) · [Content Moderation](#content-moderation) · [AI Rules](#ai-rules) · [Regular (non-AI) Rules](#regular-non-ai-rules) · [Spam vs Scams](#spam-vs-scams) · [Sentiment](#sentiment)
...
- **Source** (_optional_) — The source of the knowledge, this is used for citations and should be a URL.  
...
<Badge text="Tip" variant="tip" /> After writing or pasting an Answer, click on the "Generate Questions" button to generate example questions automatically.

<Badge text="Note" variant="tip" /> When an Entry is set to 'FAQ mode', the bot will literally paste the 'Answer' with no AI-generation. Individual Entries can also be turned off (e.g. for maintenance, or when needing an update).
...
**In C12s:** Per-channel / category / group settings that **override** global AI defaults. You can change **Personality**, **Read/Write/Learn** (*see [Agent](#agent)*), and other behaviors for specific spaces.
...
**In C12s:** A `/report` command any user can use to alert the staff of an ill-intended message or user.
Once a report is submitted by a user, the content of the reported message is analysed against the Community Guidelines, and if an appropriate action is warranted, our AI will automatically execute it.
This feature is available in Beta, and is off by default. Make sure to submit clear directives for the users to follow and the AI to analyse against (when reported).
